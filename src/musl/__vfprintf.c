...
static const unsigned char __libc("states") states[]['z'-'A'+1] = {
	{ /* 0: bare types */
		S('d') = INT, S('i') = INT,
...
static void __libc() pop_arg(union arg *arg, int type, va_list *ap)
{
	switch (type) {
...
inline static void __libc() out(FILE *f, const char *s, size_t l)
{
	if (!ferror(f)) __fwritex((void *)s, l, f);
}

static void __libc() pad(FILE *f, char c, int w, int l, int fl)
{
	char pad[256];
...
static const char __libc("xdigits") xdigits[16] = {
	"0123456789ABCDEF"
};

inline static char* __libc() fmt_x(uintmax_t x, char *s, int lower)
{
	for (; x; x>>=4) *--s = xdigits[(x&15)]|lower;
	return s;
}

inline static char* __libc() fmt_o(uintmax_t x, char *s)
{
	for (; x; x>>=3) *--s = '0' + (x&7);
	return s;
}

static char* __libc() fmt_u(uintmax_t x, char *s)
{
	unsigned long y;
...
static int __libc() fmt_fp(FILE *f, long double y, int w, int p, int fl, int t, int ps)
{
...
static int __libc() getint(char **s) {
	int i;
	for (i=0; isdigit(**s); (*s)++) {
...
static int __libc() printf_core(FILE *f, const char *fmt, va_list *ap, union arg *nl_arg, int *nl_type)
{
	char *a, *z, *s=(char *)fmt;
	unsigned l10n=0, fl;
	int w, p, xp;
...
int __libc() __vfprintf(FILE *restrict f, const char *restrict fmt, va_list ap)
{
	va_list ap2;
