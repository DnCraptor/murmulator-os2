...
void __time_critical_func() dma_handler_VGA() {
    ...
    uint8_t* input_buffer_8bit = input_buffer + y * graphics_buffer_width;
...
#define div_factor (2)
    ...
    switch (graphics_mode) {
        case GRAPHICSMODE_DEFAULT:
            ...
            for(int m = 1+(screen_line % 3 == 0 ? 0 : 0); m--;) {
                for (int i = width; i--;) {
...
uint32_t get_console_width() {
    ...
    return graphics_buffer_width / get_font_width();
}
uint32_t get_console_height() {
    ...
    return graphics_buffer_height / get_font_height();
}
uint8_t* get_buffer() {
    return text_buffer;
}
