...
50  DIM S(4),C$(32,14),M1(14),M2(14),C1(14),C2(14),M(14)
60  REM "UNION INFO ON LIKELY CONFEDERATE STRATEGY"
70  S(1)=25 : S(2)=25 : S(3)=25 : S(4)=25
82  REM "READ HISTORICAL DATA.""
84  FOR D=1 TO 14
86  READ C$()(D): READ M1(D): READ M2(D)
87  READ C1(D): READ C2(D): READ M(D)
88  NEXT D
99  REM "D=RND(-1)"
100  PRINT: PRINT "DO YOU WANT INSTRUCTIONS";
110  INPUT X$
120  IF X$="YES" OR X$="yes" THEN 160
130  IF X$="NO" OR X$="no" THEN 370
140  PRINT "YES OR NO -- ";
150  GOTO 110
...
390  INPUT B$
400  IF B$="YES" OR B$="yes" THEN 430
410  IF B$ <> "NO" AND B$ <> "no" THEN 380
...
570  INPUT X$
580  IF X$="YES" OR X$="yes" THEN 600
590  IF X$ <> "NO" AND X$ <> "no" THEN 560
...
1030  IF B$ <> "YES" AND B$ <> "yes" THEN 1050
...
1100  PRINT "THIS IS THE BATTLE OF ";CT$
1110  IF X$="NO" OR X$="no" THEN 1150
...
1140  PRINT CT$;" INSTANT REPLAY"
1150  PRINT
1160  PRINT " ","CONFEDERACY"," UNION"
1170  PRINT "MEN      ",#5; " ",M5," ",M6
1180  PRINT "MONEY    ",#5;"$";D(1),"$";D(2)
1190  PRINT "INFLATION",#5;" ";I1+15;"%"," ";I2;"%"
1195  PRINT
1200  REM "- ONLY IN PRINTOUT IS CONFED INFLATION = I1+15%""
1210  REM "- IF TWO GENERALS, INPUT CONFED. FIRST"
1220  FOR I=1 TO D
1230  IF B$ <> "YES" AND B$ <> "yes" THEN 1260
...
1700  IF B$ <> "YES" AND B$ <> "yes" THEN 1760
1710  LET O(Z)=O
1720  NEXT Z
1730  LET O2=O(2)
1740  LET O=O(1)
...
1860  IF B$ <> "YES" AND B$ <> "yes" THEN 1910
1870  FOR I=1 TO 2
...
2280  PRINT "CASUALTIES",#6, C5,C6
2290  PRINT "DESERTIONS",#6, INT(E),INT(E2)
2300  PRINT
2310  IF B$ <> "YES" AND B$ <> "yes" THEN 2350
2320  PRINT "COMPARED TO THE ACTUAL CASUALTIES AT ";CT$
...
2420  PRINT "THE UNION WINS ";CT$
2430  GOTO 2600
2440  PRINT "THE CONFEDERACY WINS "; CT$
2450  GOTO 2660
...
3000  PRINT "HISTORICAL LOSSES",#5, INT(P1+.5),INT(P2+.5)
3010  PRINT "SIMULATED LOSSES ",#5, INT(T1+.5),INT(T2+.5)
3020  PRINT
3030  PRINT "    % OF ORIGINAL",INT(100*(T1/P1)+.5),INT(100*(T2/P2)+.5)
3040  IF B$="YES" OR B$="yes" THEN 3100
3050  PRINT
