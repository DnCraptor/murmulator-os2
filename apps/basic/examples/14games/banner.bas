...
29 PRINT "STATEMENT";
30 INPUT A$
35 INPUT "SET PAGE, THEN PRESS ENTER TO START.";O$
40 A=A$(1)
50 REM
60 REM
70 FOR T=1 TO LEN(A$)
80 P$=A$(T, T)
81 IF P$>=97 AND P$<=122 THEN P$=(P$)-32
90 FOR O=1 TO 50
95 READ S$: FOR K=1 TO 7: READ S(K): NEXT
96 IF P$=" " THEN 812
100 IF P$=S$ THEN 200
120 NEXT O
200 RESTORE
...
230 IF POW(2,K)<S(U) THEN 270
240 J(9-K)=0
250 GOTO 280
270 J(9-K)=1: S(U)=S(U)-POW(2,K)
272 IF S(U)=1 THEN 815
280 NEXT K
445 FOR T1=1 TO X
447 TAB((63-4.5*Y)*G1/(LEN(X$))+1)
450 FOR B=1 TO F(U)
