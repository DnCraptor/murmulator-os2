10 REM "LUNAR from 101 BASIC games"
20 REM "Ported to Stefan's BASIC in 2021" 
25 REM
27 IF NOT USR(0,3) THEN PRINT "THIS PROGRAM NEEDS FLOAT": END
...
150 PRINT #4;L,#3;INT(A);"+";#4;INT(5280*(A-INT(A))),#7, 3600*V,#6,M-N;
155 INPUT K:T=10 
...
288 GOTO 440
300 PRINT "SORRY THERE WHERE NO SURVIVORS. YOU BLEW IT!"
...
400 IF V>0 THEN 370
410 GOTO 160
420 Q=S*K/M: J=V+G*S+Z*(-Q-Q*Q/2-POW(Q,3)/3-POW(Q,4)/4-POW(Q,5)/5)
430 I=A-G*S*S/2-V*S+Z*S*(Q/2+Q*Q/6+POW(Q,3)/12+POW(Q,4)/20+POW(Q,5)/30):RETURN
