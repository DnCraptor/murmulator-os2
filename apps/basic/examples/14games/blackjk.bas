...
720 REM PRINT MID$(D$,,3);
721 PRINT D$(3*X-2, 3*X);
730 PRINT "  ";
740 RETURN
750 REM--ALTERNATIVE PRINTING ROUTINE
760 REM PRINT " ";MID$(D$,3*X-1,2);
761 PRINT " ";D$(3*X-1, 3*X);
770 PRINT "   ";
780 RETURN
...
1310 PRINT
1320 A0=Q(I): GOSUB 3400
1325 PRINT "TOTAL IS",A0
1330 RETURN
1400 REM--SUBROUTINE TO READ REPLY
1410 REM  I$ DEFINED ELSEWHERE
1420 INPUT H$: H$=H$(1,1)
1430 FOR H=1 TO H1 STEP 2
1440 IF H$=I$(H,H) THEN 1480
1450 NEXT H
1460 PRINT "TYPE ";I$(1,H1);" OR ";I$(H1,H1+1);" PLEASE";
1470 GOTO 1420
1480 H=(H+1)/2
1490 RETURN
...
1620 INPUT H$
1630 IF H$(1,1)="N" OR H$(1,1)="n" THEN 1760
...
3000 GOTO 3140
3010 PRINT "DEALER HAS A ";D$(3*P(D1,2)-2,3*P(D1,2));" CONCEALED ";
3020 I=D1
3030 A0=Q(I): GOSUB 3400
3035 PRINT "FOR A TOTAL OF ";A0
3040 IF A0>16 THEN 3130
3050 PRINT "DRAWS";
3060 GOSUB 100
3070 GOSUB 750
3080 GOSUB 1100
3090 A0=Q: GOSUB 3400
3095 IF Q>0 AND A0<17 THEN 3060
3100 Q(I)=Q-(Q<0)/2
3110 IF Q<0 THEN 3140
3120 A0=Q: GOSUB 3400
3125 PRINT "---TOTAL IS ";A0
3130 PRINT
...
3165 PRINT
3170 FOR I=1 TO N
3180 A0=Q(I): GOSUB 3400
3182 A1=Q(I+D1): GOSUB 3410
3184 A2=Q(D1): GOSUB 3420
3186 S(I)=S(I)+B(I)*SGN(A0-A2)+B(I+D1)*SGN(A1-A2)
3188 B(I+D1)=0
3200 PRINT "PLAYER ";I;" ";
3210 PRINT Z$(SGN(S(I))*6+7,SGN(S(I))*6+7+5);" ";
...
3350 GOSUB 1200
3360 GOTO 1810
3400 A0=A0+11*(A0>=22): RETURN
3410 A1=A1+11*(A1>=22): RETURN
3420 A2=A2+11*(A2>=22): RETURN
