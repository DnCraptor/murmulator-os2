...
static bool file_read(FIL* fil, void* buffer, uint32_t size, const char* msg);
...
static bool mp3FileRead(music_file* mf, int16_t* dest, uint32_t len, uint32_t* written)
{
    ...
    *written = 0;
    ...
    STATUS(("mp3FileRead start: Bytes left in working buffer: %u\n", musicFileBytesLeft(mf)));
        ...
        if (state == ID_OK)
        {
            ...
            if (err) 
            {
                ...
                if (err == ERR_MP3_INDATA_UNDERFLOW) {
                        ...
                        state = ID_OK;
                } else if (err == ERR_MP3_FREE_BITRATE_SYNC) {
            ...
            else 
            {
                ...
                *written += decode_len;
                ...
                STATUS(("MP3Decode Consumed bytes: %u Generated bytes: %u\n", consumed, decode_len << 1)); 
            }
        }
    } while (state == ID_OK);
