...
static void m_save(uint8_t cmd) {
    ...
    node_t* i = lst->first;
    while (i) {
        fwrite(c_str(i->data), c_strlen(i->data), 1, f);
        fwrite("\n", 1, 1, f);
        i = i->next;
    }
    fclose(f);
...
int main(int argc, char* argv[]) {
    if (argc == 0) {
        fprintf(stderr, "ATTTENTION! BROKEN EXECUTION CONTEXT!\n");
        return -1;
    }
    if (argc != 2) {
        fprintf(stderr, "Unexpected number of arguemts: %d (expected: 1)\n", argc - 1);
        return 1;
    }
