...
static void* fi_allocator(void) {
    file_info_t* res = (file_info_t*)calloc(sizeof(file_info_t), 1);
    res->s_name = new_string_v();
    return res;
}

static void fi_deallocator(void* p) {
    delete_string(((file_info_t*)p)->s_name);
    free(p);
}
...
static bool m_unlink_recursive(const char *path) {
    struct stat st;
    ...
    while ((ent = readdir(dir)) != NULL) {
        ...
        construct_full_name(s_path, path, name);

        printf(".");

        if (!m_unlink_recursive(s_path->p)) {
        ...
        if (mselsz) {
            ...
            while(i) {
                ...
                if ( i->data ) {
                    ...
                    result = fi->fattr & AM_DIR ? m_unlink_recursive(s_path->p) : (unlink(s_path->p) == 0);
                    printf(".");
                    if (!result) break;
            ...
            result = fp->fattr & AM_DIR ? m_unlink_recursive(s_path->p) : (unlink(s_path->p) == 0);
        }
        if (!result) {
...
inline static bool m_copy(const char* path, const char* dest) {
    ...
    while ((br = fread(buff, 1, 512, f1)) > 0) {
        size_t bw = fwrite(buff, 1, br, f2);
        if (bw != br) {
            ok = false;
            break;
        }
...
static bool m_copy_recursive(const char* path, const char* dest) {
    DIR* dir = opendir(path);
    if (!dir)
        return false;
    ...
    while ((ent = readdir(dir)) != NULL) {
        ...
        printf(".");
        construct_full_name(s_path, path, name);
        construct_full_name(s_dest, dest, name);
        bool ok;
        // TODO:
        //struct stat st;
        //if (stat(path, &st) == 0) {
        //    S_ISDIR(st.st_mode)
        //}
        if (ent->ff_info.fattrib & AM_DIR) {
            ok = m_copy_recursive(s_path->p, s_dest->p);
        } else {
            ok = m_copy(s_path->p, s_dest->p);
        }
        if (!ok) {
            delete_string(s_path);
...
static void draw_cmd_line(void) {
    ...
    fprintf(stderr, "[%s]", cd ? cd : "NULL");
    graphics_set_con_color(7, 0);
    fprintf(stderr, "> %s", s_cmd->p ? s_cmd->p : "NULL");
...
inline static void save_rc() {
    ...
    fwrite(left_panel, 1, sizeof(file_panel_desc_t), pfh);
    fwrite(&sz0, 1, sizeof(size_t), pfh);
    fwrite(left_panel->s_path->p, 1, sz0, pfh);
    fwrite(right_panel, 1, sizeof(file_panel_desc_t), pfh);
    sz0 = right_panel->s_path->size + 1;
    fwrite(&sz0, 1, sizeof(size_t), pfh);
    fwrite(right_panel->s_path->p, 1, sz0, pfh);
    bool left_selected = (psp == left_panel);
    fwrite(&left_selected, 1, sizeof(left_selected), pfh);
    fwrite(&hidePannels, 1, sizeof(hidePannels), pfh);
    fclose(pfh);
    free(mc_rc_file);
}
...
inline static bool initi_from_rc(cmd_ctx_t* ctx) {
    ...
    size_t rb;
    if (fread(left_panel, 1, sizeof(file_panel_desc_t), pfh) != sizeof(file_panel_desc_t)) goto r2;
    size_t panel_path_sz;
    if (fread(&panel_path_sz, 1, sizeof(size_t), pfh) != sizeof(size_t)) goto r2;
    left_panel->s_path = preserv_string;
    string_reseve(left_panel->s_path, panel_path_sz);
    if (fread(left_panel->s_path->p, 1, panel_path_sz, pfh) != panel_path_sz) goto r2;
    left_panel->s_path->size = panel_path_sz - 1;
    preserv_string = right_panel->s_path;
    if (fread(right_panel, 1, sizeof(file_panel_desc_t), pfh) != sizeof(file_panel_desc_t)) goto r2;
    if (fread(&panel_path_sz, 1, sizeof(size_t), pfh) != sizeof(size_t)) goto r2;
    right_panel->s_path = preserv_string;
    string_reseve(right_panel->s_path, panel_path_sz);
    if (fread(right_panel->s_path->p, 1, panel_path_sz, pfh) != panel_path_sz) goto r2;
    right_panel->s_path->size = panel_path_sz - 1;
    bool left_selected;
    if (fread(&left_selected, 1, sizeof(left_selected), pfh) != sizeof(left_selected)) goto r2;
    if (!left_selected) psp = right_panel;
    if (fread(&hidePannels, 1, sizeof(hidePannels), pfh) != sizeof(hidePannels)) goto r2;
    res = true;
r2:
    fclose(pfh);
    free(mc_rc_file);
